...
class PACSGeneralBase(Dataset):
    ...
    def _prepare(self):
        self.root = os.getcwd() + f"/data/PACS/{self.domain}/{self.content}/"
        namelist = os.listdir(self.root)
        with open(os.getcwd() + f"/data/meta/{self.domain}_{self.content}.txt", "w") as f:
            for name in namelist:
                if name != ".ready":
                    f.write(name + "\n")
        self._data_path = os.getcwd() + f"/data/meta/{self.domain}_{self.content}.txt"
